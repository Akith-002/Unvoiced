"""
ASL Sign Language Classification Training for Kaggle - FIXED VERSION
...
def setup_environment():
    """Setup and check Kaggle environment"""
...
def calculate_class_weights(dataset, class_names):
    ...
    print(f"Class distribution calculated for {len(class_names)} classes")
...
def train_model(model, base_model, train_ds, val_ds, class_weights, epochs=30, output_dir='models'):
    ...
    print("🔥 Starting Phase 2: Fine-tune entire model")
...
def main():
    ...
    BATCH_SIZE = 64 if has_gpu else 32  # Larger batch size for GPU
